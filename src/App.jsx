...
function App() {
  ...
  const handleForcePasswordSubmit = async (e) => {
    e.preventDefault()
    if (forceNewPassword !== forceConfirmPassword) return showToast(t('alertPassMatch'), 'error')
    if (forceNewPassword.length < 6) return showToast(t('passwordMinLength'), 'error')
      ...
      <div data-theme={currentTheme} className="min-h-screen flex items-center justify-center bg-base-300 p-4 font-sans">
        <div className="card w-full max-w-md shadow-2xl bg-base-100 border border-base-200">
          <form onSubmit={handleForcePasswordSubmit} className="card-body p-6">
            <h2 className="card-title text-xl font-black text-error justify-center">🔒 {t('forcePasswordTitle')}</h2>
            <input type="password" required placeholder={t('lblNewPass')} className="input input-bordered w-full" value={forceNewPassword} onChange={(e) => setForceNewPassword(e.target.value)} />
            <input type="password" required placeholder={t('lblConfirmPass')} className="input input-bordered w-full" value={forceConfirmPassword} onChange={(e) => setForceConfirmPassword(e.target.value)} />
            <button type="submit" disabled={forceLoading} className="btn btn-error text-white font-bold w-full mt-4">{t('btnUpdatePass')}</button>
          </form>
        </div>
      </div>
    )
