...
export default function RecordManager({ session }) {
  ...
  const handleGenerate = () => {
    ...
    const lines = selectedRecords.map(rec => {
      return `${t('datePrefix')} : ${rec.date} - RM ${parseFloat(rec.amount).toFixed(2)}`
    })
  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="flex flex-col gap-1 border-b border-base-100 pb-4">
        ...
        <p className="text-sm opacity-60">{lang === 'ms' ? 'Tambah entri transaksi baharu dan jana format output kewangan.' : 'Add new transaction entries and generate formatted financial outputs.'}</p>
      </div>
        ...
        <div className="card bg-base-100 shadow-xl p-6 border border-base-200 h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="form-control">
              ...
              <input type="date" required className="input input-bordered w-full text-base" value={date} onChange={(e) => setDate(e.target.value)} />
            </div>
            <div className="form-control">
              <label className="label-text font-semibold mb-1">{t('amount')}</label>
              <input type="number" step="0.01" required placeholder="0.00" className="input input-bordered w-full text-base" value={amount} onChange={(e) => setAmount(e.target.value)} />
            </div>
            <button type="submit" disabled={loading} className="btn btn-primary w-full mt-2">
              {loading ? <span className="loading loading-spinner"></span> : t('saveRecord')}
            </button>
          </form>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              {checkedIds.length > 0 && (
                <div className="bg-base-200 px-3 py-1.5 rounded-xl border border-base-300 text-sm flex items-center gap-1.5">
                  ...
                  <button type="button" onClick={handleCopyTotalNumberOnly} className="btn btn-ghost btn-xs btn-circle"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 17.25v3.375c0 .621-.504 1.125-1.125 1.125h-9.75a1.125 1.125 0 0 1-1.125-1.125V7.875c0-.621.504-1.125 1.125-1.125H6.75a9.06 9.06 0 0 1 1.5.124m7.5 10.376A8.965 8.965 0 0 0 12 12.75c-.497 0-.982.04-1.455.12l-.179.03m1.608-3.033a13.96 13.96 0 0 1 2.5.553m-.553-1.376a13.518 13.518 0 0 1 4.722 3.208 13.518 13.518 0 0 1 3.208 4.722m-3.208-4.722a13.48 13.48 0 0 0-4.722-3.208M4.5 10.5h11.25c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125H4.5A1.125 1.125 0 0 1 3.375 21.375v-9.75c0-.621.504-1.125 1.125-1.125Z" /></svg></button>
                </div>
              )}
              <div className="flex gap-2">
                <button onClick={handleGenerate} disabled={checkedIds.length === 0 || loading} className="btn btn-sm btn-accent text-white">{t('generate')} ({checkedIds.length})</button>
                <button onClick={handleDeleteChecked} disabled={checkedIds.length === 0 || loading} className="btn btn-sm btn-error text-white">{t('delete')}</button>
              </div>
            </div>
          </div>

          {records.length === 0 ? (
            <p className="text-center py-8 opacity-60">{t('noRecords')}</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="table table-zebra w-full">
                <thead>
                  <tr>
                    <th className="w-12"><input type="checkbox" className="checkbox checkbox-primary checkbox-sm" onChange={handleCheckAll} checked={records.length > 0 && checkedIds.length === records.length} /></th>
                ...
                <tbody>
                  {records.map((rec, index) => (
                    <tr key={rec.id} className={checkedIds.includes(rec.id) ? 'bg-base-200/50' : ''}>
                      <td><input type="checkbox" className="checkbox checkbox-primary checkbox-sm" checked={checkedIds.includes(rec.id)} onChange={() => handleCheckRow(rec.id)} /></td>
      ...
      {generatedText && (
        <div className="card bg-base-100 shadow-xl p-6 border border-base-200 w-full animate-fade-in">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={handleCopyToClipboard} className="btn btn-sm btn-outline btn-accent">{t('copyText')}</button>
          </div>
          <div className="bg-base-300 p-4 rounded-xl font-mono text-sm leading-relaxed whitespace-pre-wrap border border-base-200 select-all">{generatedText}</div>
        </div>
      )}
        ...
        <div className="modal modal-open z-50">
          <div className="modal-box max-w-sm border border-base-200 shadow-2xl rounded-2xl p-6">
            ...
            <form onSubmit={handleUpdateRecord} className="space-y-4">
              <div className="form-control"><label className="label-text font-semibold mb-1">{t('date')}</label><input type="date" required className="input input-bordered w-full text-base" value={editDate} onChange={(e) => setEditDate(e.target.value)} /></div>
              <div className="form-control"><label className="label-text font-semibold mb-1">{t('amount')}</label><input type="number" step="0.01" required className="input input-bordered w-full text-base" value={editAmount} onChange={(e) => setEditAmount(e.target.value)} /></div>
              <div className="modal-action gap-2 pt-2">
                <button type="button" className="btn btn-sm btn-ghost" onClick={() => setEditingRecord(null)}>{t('cancel')}</button>
                <button type="submit" disabled={loading} className="btn btn-sm btn-info text-white font-bold px-4">{loading ? t('saving') : t('saveChanges')}</button>
              </div>
            </form>
          </div>
        </div>
